...
插件不会后台扫描整个 Vault。每轮上下文有文件数、单文件字符数与总字符数上限；本轮实际内容可查看和移除。远程接口只允许 HTTPS，本机回环地址可用 HTTP。错误信息在进入通知与历史前会脱敏。
...
VaultMuse never indexes the whole vault in the background. Per-turn file and character limits are enforced, remote endpoints require HTTPS, loopback HTTP remains available for local models, and provider errors are redacted before display or persistence. Your chosen model provider may still retain explicitly sent content under its own policy.
